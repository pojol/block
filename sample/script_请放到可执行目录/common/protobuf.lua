...
function _reader:real(key)
	return PBC._rmessage_real(self._CObj , key , 0)
end

function _reader:string(key)
	return PBC._rmessage_string(self._CObj , key , 0)
end

function _reader:bool(key)
	return PBC._rmessage_int(self._CObj , key , 0) ~= 0
end

function _reader:message(key, message_type)
	local rmessage = PBC._rmessage_message(self._CObj , key , 0)
	if rmessage then
		local v = {
			_CObj = rmessage,
			_CType = message_type,
...
function _reader:int(key)
	return PBC._rmessage_int(self._CObj , key , 0)
end

function _reader:real_repeated(key)
	local cobj = self._CObj
	local n = PBC._rmessage_size(cobj , key)
	local ret = {}
	for i=0,n-1 do
		tinsert(ret,  PBC._rmessage_real(cobj , key , i))
	end
	return ret
end
...
function _reader:string_repeated(key)
	...
	local ret = {}
	for i=0,n-1 do
		tinsert(ret,  PBC._rmessage_string(cobj , key , i))
	end
	return ret
end
...
function _reader:bool_repeated(key)
	...
	local ret = {}
	for i=0,n-1 do
		tinsert(ret,  PBC._rmessage_int(cobj , key , i) ~= 0)
	end
	return ret
end
...
function _reader:message_repeated(key, message_type)
	...
	for i=0,n-1 do
		local m = {
			_CObj = PBC._rmessage_message(cobj , key , i),
			_CType = message_type,
			_Parent = self,
		}
...
function _reader:int_repeated(key)
	...
	local ret = {}
	for i=0,n-1 do
		tinsert(ret,  PBC._rmessage_int(cobj , key , i))
	end
	return ret
end
...
function _decode_type_meta:__index(key)
	local t, msg = PBC._env_type(P, self._CType, key)
...
local function decode_message( message , buffer, length)
	local rmessage = PBC._rmessage_new(P, message, buffer, length)
	if rmessage then
		local self = {
			_CObj = rmessage,
			_CType = message,
		}
		PBC._add_rmessage(GC,rmessage)
...
local _writer = {
	real = PBC._wmessage_real,
	enum = PBC._wmessage_string,
	string = PBC._wmessage_string,
	int = PBC._wmessage_int,
}

function _writer:bool(k,v)
	PBC._wmessage_int(self, k, v and 1 or 0)
end

function _writer:message(k, v , message_type)
	local submessage = PBC._wmessage_message(self, k)
	encode_message(submessage, message_type, v)
end

function _writer:real_repeated(k,v)
	for _,v in ipairs(v) do
		PBC._wmessage_real(self,k,v)
	end
end
...
function M.encode( message, t , func , ...)
	local encoder = PBC._wmessage_new(P, message)
	assert(encoder ,  message)
	encode_message(encoder, message, t)
	if func then
		local buffer, len = PBC._wmessage_buffer(encoder)
		local ret = func(buffer, len, ...)
		PBC._wmessage_delete(encoder)
		return ret
	else
		local s = PBC._wmessage_buffer_string(encoder)
		PBC._wmessage_delete(encoder)
		return s
	end
end
...
local function _pattern_create(pattern)
	...
	local lua = {}
	for v in iter do
		local tidx = PBC._env_type(P, message, v)
		local t = _pattern_type[tidx]
		assert(t,tidx)
		tinsert(cpat,v .. " " .. t[1])
		tinsert(lua,t[2])
	end
	local cobj = PBC._pattern_new(P, message , "@" .. table.concat(cpat," "))
	if cobj == nil then
		return
	end
	PBC._add_pattern(GC, cobj)
	local pat = {
		CObj = cobj,
		format = table.concat(lua),
		size = 0
	}
	pat.size = PBC._pattern_size(pat.format)
...
function M.unpack(pattern, buffer, length)
	...
	return PBC._pattern_unpack(pat.CObj , pat.format, pat.size, buffer, length)
end

function M.pack(pattern, ...)
	local pat = _pattern_cache[pattern]
	return PBC._pattern_pack(pat.CObj, pat.format, pat.size , ...)
end

function M.check(typename , field)
	if field == nil then
		return PBC._env_type(P,typename)
	else
		return PBC._env_type(P,typename,field) ~=0
	end
end
...
function M.decode(typename, buffer, length)
	local ret = {}
	local ok = PBC._decode(P, decode_message_cb , ret , typename, buffer, length)
	if ok then
		return setmetatable(ret , default_table(typename))
	else
		return false , PBC._last_error(P)
	end
end
...
local function expand(tbl)
	...
	tbl[1] , tbl[2] = nil , nil
	assert(PBC._decode(P, decode_message_cb , tbl , typename, buffer), typename)
...
local function set_default(typename, tbl)
	for k,v in pairs(tbl) do
		if type(v) == "table" then
			local t, msg = PBC._env_type(P, typename, k)
			if t == 6 then
				set_default(msg, v)
			elseif t == 128+6 then
				for _,v in ipairs(v) do
...
function M.enum_id(enum_type, enum_name)
	return PBC._env_enum_id(P, enum_type, enum_name)
end

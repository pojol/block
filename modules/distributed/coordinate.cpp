...
void gsf::modules::CoodinatorModule::event_adjust_module_weight(const gsf::ArgsPtr &args, gsf::CallbackFunc callback)
{
	...
	adjust_module_weight(_port, _module, _module_id, _weight);
}
...
void gsf::modules::CoodinatorModule::event_regist(const gsf::ArgsPtr &args, gsf::CallbackFunc callback)
{
	...
	for (int i = 0; i < _count; ++i)
	{
		...
		adjust_module_weight(_port, _module_name, _module_id, 0);
	}
...
void gsf::modules::CoodinatorModule::adjust_module_weight(int32_t port, const std::string &module_name, gsf::ModuleID module_id, int32_t weight)
{
	auto _itr = node_id_map_.find(port);
